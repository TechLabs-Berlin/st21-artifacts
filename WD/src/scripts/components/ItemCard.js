...
const ItemCard = ({ item, onClick }) => {
  ...
  const [isFavorite, setIsFavorite] = useState(userInformation.favorites.some(favItem => item.key === favItem.key))
        ...
        .then((snapshot) => {
          snapshot.forEach((childSnapshot) => {
            if (childSnapshot.val().key === item.key) {
              ...
              newUserInformation.favorites = newUserInformation.favorites.filter(favItem => favItem.key !== item.key)
        ...
        <div className={`item-card ${onClick ? 'link' : '' }`} onClick={onClickItem}>
          ...
          <div className="card-body">
            ...
            <button
              ...
              className={isFavorite ? "item-heart-button-active" : "item-heart-button"}
            >
